...
xcb::intern_atom_cookie(const char *__name)
{
...
xcb::intern_atom_reply(xcb_intern_atom_cookie_t __cookie)
{
    return xcb_intern_atom_reply(_conn, __cookie, nullptr);
}

xcb_atom_t 
xcb::intern_atom(const char *__name)
{
...
bool
xcb::window_exists(uint32_t __w)
{
...
void
xcb::window_stack(uint32_t __window1, uint32_t __window2, uint32_t __mode)
{
...
bool
xcb::is_flag_set(unsigned int __f)
{
    return ( _flags & ( 1ULL << __f )) != 0;
}

void
xcb::set_flag(unsigned int __f)
{
    _flags |= 1ULL << __f;
}

void
xcb::clear_flag(unsigned int __f)
{
    _flags &= ~(1ULL << __f);
}

void
xcb::toggle_flag(unsigned int __f)
{
    _flags ^= 1ULL << __f;
}

xcb::xcb(xcb_connection_t *__conn, xcb_screen_t *__s)
    : _s(__s)
{
